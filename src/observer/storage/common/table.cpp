...
RC Table::drop(std::string& data_path)
{
  std::string table_meta_path = table_meta_file(data_path.c_str(), name());

  if (unlink(table_meta_path.c_str()) != 0) {
    LOG_ERROR("Failed to remove table meta files [%s] and related links. errono: [%s].", table_meta_path.c_str(), strerror(errno));
    return RC::GENERIC_ERROR;
  }

  std::string table_data_path = table_data_file(data_path.c_str(), name());

  if (unlink(table_data_path.c_str()) != 0) {
    LOG_ERROR("Failed to remove table data files [%s] and related links. errono: [%s].", table_data_path.c_str(), strerror(errno));
    return RC::GENERIC_ERROR;
  }

  int table_index_num = table_meta_.index_num();
  for (int i = 0; i < table_index_num; ++i) {
    IndexMeta index_meta = indexes_[i]->index_meta();
    std::string index_file_path = table_index_file(data_path.c_str(), name(), index_meta.name());
    if (unlink(index_file_path.c_str()) != 0) {
      LOG_ERROR("Failed to remove table data files [%s] and related links. errono: [%s].", index_file_path.c_str(), strerror(errno));
      return RC::GENERIC_ERROR;
    }
  }
  return RC::SUCCESS;
